...
- 71 deterministic tests, build, typecheck, configuration validation, package dry-run and dependency audit successful;
...
- ChatCOM v0.2.0 is formally tagged and released; v0.3.0 is the current operational baseline.

The next work must begin with the v0.3.0 read-only post-publication directive in
`CODEX-CHATCOM-PROMPT.md`. Every later real diagnostic or relay run still
requires explicit authorization for that mission.
...
- Treat the formal v0.2.0 release and the v0.3.0 MCP baseline as immutable history.
