...
test("version ordering rejects downgrades and constructs the official feed", () => {
  ...
  assert.equal(compareVersions("1.0.0-rc.5", "1.0.0-rc.6"), -1);
  assert.equal(buildUpdateFeedUrl("1.0.0-rc.5"), "https://update.electronjs.org/NoisyBoyFR/ChatCOM/win32-x64/1.0.0-rc.5");
  assert.equal(isOfficialUpdateFeedUrl(buildUpdateFeedUrl("1.0.0-rc.5"), "1.0.0-rc.5"), true);
  assert.equal(isOfficialUpdateFeedUrl("http://update.electronjs.org/NoisyBoyFR/ChatCOM/win32-x64/1.0.0-rc.5", "1.0.0-rc.5"), false);
  const preview = buildUpdateFeedUrl("1.0.0-rc.5", "preview");
  assert.match(preview, /noisyboyfr\.github\.io\/ChatCOM\/updates\/preview\/win32\/x64/u);
  assert.equal(isOfficialUpdateFeedUrl(preview, "1.0.0-rc.5", "preview"), true);
});
...
test("updater delays startup, serializes checks, waits for clean relay state, and never forces restart", async () => {
  ...
  adapter.emit("update-downloaded", {}, "safe notes", "ChatCOM Desktop 1.0.0-rc.6", new Date("2026-08-21T00:00:00.000Z"), "http://127.0.0.1:43127/preview/win32/x64");
  await new Promise((resolve) => setImmediate(resolve));
  ...
  adapter.emit("update-downloaded", {}, "", "ChatCOM Desktop 1.0.0-rc.5", new Date("2026-08-21T00:00:00.000Z"), "http://127.0.0.1:43127/preview/win32/x64");
  await new Promise((resolve) => setImmediate(resolve));
...
test("update-downloaded without complete official metadata never reaches READY", async () => {
  const adapter = new FakeUpdater();
  const controller = new UpdaterController({ adapter, currentVersion: "1.0.0-rc.5", channel: "stable", publicUpdatesEnabled: true, policyEnabled: true, loadManifest: async () => ({}), relayState: () => ({ activity: "IDLE", cleanupConfirmed: true }) });
  controller.start();
  adapter.emit("update-downloaded", {}, "notes", undefined, new Date("2026-08-21T00:00:00.000Z"), "https://update.electronjs.org/NoisyBoyFR/ChatCOM/win32-x64/1.0.0-rc.5");
  await new Promise((resolve) => setImmediate(resolve));
...
test("disabled or unsigned update checks never contact the updater", async () => {
  const adapter = new FakeUpdater();
  const controller = new UpdaterController({ adapter, currentVersion: "1.0.0-rc.5", channel: "preview", publicUpdatesEnabled: false, policyEnabled: false });
