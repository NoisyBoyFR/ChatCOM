...
test("runs three automatic transmissions and stops before the second Codex mission", async () => {
  ...
  const schemas = agent.turns.map(({ outputSchema }) => outputSchema as { properties: Record<string, { enum?: readonly unknown[] }>; anyOf?: readonly { properties: Record<string, { enum?: readonly unknown[] }> }[] });
  assert.equal(schemas.length, 3);
  assert.deepEqual(schemas[0].properties.version.enum, ["1.0"]);
  assert.deepEqual(schemas[0].properties.session_id.enum, [sessionId]);
  assert.deepEqual(schemas[0].properties.sequence.enum, [1]);
  assert.deepEqual(schemas[0].properties.sender.enum, ["WORK_LOCAL"]);
  assert.deepEqual(schemas[0].properties.recipient.enum, ["CODEX_LOCAL"]);
  assert.deepEqual(schemas[0].properties.type.enum, ["MISSION"]);
  assert.deepEqual(schemas[0].properties.correlation_id.enum, [sessionId]);
  assert.deepEqual(schemas[1].properties.sequence.enum, [2]);
  assert.deepEqual(schemas[1].properties.sender.enum, ["CODEX_LOCAL"]);
  assert.deepEqual(schemas[1].properties.recipient.enum, ["WORK_LOCAL"]);
  assert.deepEqual(schemas[1].properties.type.enum, ["REPORT"]);
  assert.deepEqual(schemas[1].properties.correlation_id.enum, [messages[0].message_id]);
  const normalNextSchema = schemas[2].anyOf?.[0];
  assert.ok(normalNextSchema);
  assert.deepEqual(normalNextSchema.properties.sequence.enum, [3]);
  assert.deepEqual(normalNextSchema.properties.type.enum, ["NEXT_PROMPT"]);
  assert.deepEqual(normalNextSchema.properties.correlation_id.enum, [messages[1].message_id]);
